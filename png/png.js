...
var CrcStream = require('./CrcStream');
...
// Could disable C++ functionality, with no reference to the module.
//  If it does not load the module, it could use the JavaScript code instead.
...
class PNG extends Evented_Class {
    constructor(spec) {
        super();
        ...
        if (spec.buffer) {
            this.load_from_buffer(spec.buffer);
        }

        // definitely want to be able to load from different simple formats.
        //  pixel buffers.



        // if (spec.rgba_buffer)
        //  that's the unfiltered scanlines?

        // Loading from rgb / rgba?
    }
    ...
    'init_scanlines_buffer'(size, color_type, bit_depth) {
    ...
    'set_scanline_filter_all_rows'(scanline_filter_num) {
    ...
    'set_scanline_filter'(scanline_num, scanline_filter_byte) {
    ...
    'set_scanline_filter_byte_all_rows'(scanline_filter_num) {
        var h = this.size[1];
        for (var y = 0; y < h; y++) {
    ...
    'set_scanline_filter_byte'(scanline_num, scanline_filter_byte) {
        ...
        //throw 'stop';
        var h = this.size[1],
            scanline_length = this.scanline_length,
            scanlines_buffer = this.scanlines_buffer;
    ...
    'get_scanline_filter_byte'(scanline_num) {
    ...
    'get_filtered_scanline_buffer'(scanline_num) {
    ...
    // ?
    'calc_filtered_scanline_buffer'(scanline_num) {
        // may re-encode it... not sure.
    ...
    '_iterate_pixels'(pixel_callback) {
        var width = this.size[0];
        var height = this.size[1];
    ...
    'analyse_scanline_filter_method_get_compressed_size'() {
        ...
        var scanline_num, scanline_filter_byte, options = {},
            callback;
    ...
    'optimize_scanline_filters'() {
        ...
        if (method == 'deflate') {
            ...
            that.analyse_scanlines_predict_best_filters({
                'method': method
            }, function (err, arr_predicted_best_scanline_filters) {
                if (err) {
                    throw err;
                } else {
            ...
            var arr_predicted_best_scanline_filters = that.analyse_scanlines_predict_best_filters({
                'method': method
            });
            each(arr_predicted_best_scanline_filters, function (y, predicted_best_filter) {
                that.set_scanline_filter_byte(y, predicted_best_filter);
    ...
    'analyse_scanlines_predict_best_filters'() {
        ...
        if (method == 'deflate') {
            ...
            for (var y = 0; y < this.size[1]; y++) {
                //fns.push()
                fns.push([this, this.analyse_scanline_predict_best_filter, [y, {
                    'method': method
                }]]);
            }
    ...
    'analyse_scanline_predict_best_filter'(_a, _sig) {
        ...
        if (method == 'deflate') {
            ...
            fns.push([this, this.analyse_scanline_filter_method_get_compressed_size, [scanline_num, 0, {
                'method': method
            }]]);
            fns.push([this, this.analyse_scanline_filter_method_get_compressed_size, [scanline_num, 1, {
                'method': method
            }]]);
            fns.push([this, this.analyse_scanline_filter_method_get_compressed_size, [scanline_num, 2, {
                'method': method
            }]]);
            fns.push([this, this.analyse_scanline_filter_method_get_compressed_size, [scanline_num, 3, {
                'method': method
            }]]);
            fns.push([this, this.analyse_scanline_filter_method_get_compressed_size, [scanline_num, 4, {
                'method': method
            }]]);
    ...
    'ensure_unfiltered_scanlines_buffer_cpp'() {
        // _ensure_unfiltered_scanlines_buffer
    ...
    'set_color_parameters'(color_type, bit_depth) {
    ...
    'save_to_stream'(_a, _sig) {
        ...
        var do_optimize = function (callback) {
            ...
            if (optimize == 'fast' || optimize == 'best') {
                ...
                that.optimize_filter_all_scanlines({
                    'optimize': optimize
                }, function (err, res_best_filters) {
    ...
    'load_from_stream'(input_stream, callback) {
...
const load_metadata_from_stream = (stream) => {
    var data_num = 0;
...
let decode_to_pb = (buf) => new Promise((solve, jettison) => {
    let png = new PNG({
        buffer: buf
    });
    png.on('ready', async () => {
...
let decode_to_raw_obj = (buf) => new Promise((solve, jettison) => {
    let png = new PNG({
        buffer: buf
    });
    png.on('ready', async () => {
